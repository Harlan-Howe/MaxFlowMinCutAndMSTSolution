...
class MaxFlowMinCutSolver:
    def find_max_flow(self, capacity: DirectedGraph, capacity_key: str = KEY_CAPACITY) -> Tuple[DirectedGraph,
        ...
        while True:
            ...
            for i in range(len(path) - 1):
                flow.get_edge_from_u_to_v(path[i], path[i+1])[KEY_FLOW] += min_value
    ...
    def generate_residual(capacity: DirectedGraph, flow: DirectedGraph) -> DirectedGraph:
        ...
        for e_id in capacity.E:  # loop through all the edges in capacity graph...
            ...
            # build the edge or edges in residual that go with this particular pair of capacity and flow edges
            ...
            if capacity_amount > flow_amount:
                residual.add_edge(u, v, additional_info={KEY_CAPACITY: capacity_amount - flow_amount})
            if flow_amount > 0:
                residual.add_edge(v, u, additional_info={KEY_CAPACITY: flow_amount})
        return residual
    ...
    def find_nonzero_path_in_graph(graph: DirectedGraph,
        ...
        #  perhaps use a random selection/ordering when options are equivalent.... you can use "random.shuffle(name_of_list)"
