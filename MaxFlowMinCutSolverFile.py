...
class MaxFlowMinCutSolver:
    def find_max_flow(self,capacity:DirectedGraph, capacity_key:str = "capacity") -> Tuple[DirectedGraph, DirectedGraph]:
        """
        finds the maximum flow from "S" to "T" for the given graph -
        :param capacity: The directed graph in which to perform the search - it should contain a vertex labeled "S" and one
                         labeled "T".
        :param capacity_key: the key used to ask each edge for the capacity of this connection
        :return: flow - a parallel graph to capacity, with the same vertices, and edges labeled by "flow" with the amount of flow
                            through that edge
                 residual - a similar graph to capacity, with the same vertices, and edges layed out parallel and anti-parallel
                             to the original. The residual is what is used to calculate the max flow, and it is included in
                             the output to be used for finding the min cut.
        """
        # --> We'll start by initializing "flow".....
        flow:DirectedGraph = DirectedGraph(capacity.V, {})  # make a flow be a copy of the capacity's vertices, with no edges.
        ...
        self.display_graphs(capacity, flow, residual,
                       path_display)  # NOTE: this takes time to generate, so you will likely want to comment this out for a complicated graph.
    ...
    def generate_residual(self,capacity: DirectedGraph, flow: DirectedGraph) -> DirectedGraph:
        ...
        :return:
        """
        residual:DirectedGraph = DirectedGraph(capacity.V, {})  # create a new graph with the same vertices, but no edges.
        for e_id in capacity.E:  # loop through all the edges in capacity graph...
            ...
            flow_edge_id: int = flow.get_edge_from_u_to_v(u, v)
            # just checking.... this shouldn't ever happen.
            if flow_edge_id is None:
                raise AssertionError(f"edge {u}-->{v} not found in flow graph")

            flow_edge: Edge = flow.E[flow_edge_id]

            capacity_amount: int = capacity_edge["capacity"]
            flow_amount: int = flow_edge["flow"]

            # build the edge or edges in residual that go with this capacituy and flow edge
            #TODO: You write this. You'll want to use "residual.add_edge(u_id=<?>, v_id=<?>, additional_info = {"capacity": <???>})"
    ...
    def find_nonzero_path_in_graph(self,
                                   ...
                                   start_label: str = "S",
                                   end_label: str = "T",
                                   key: str = "capacity") -> List[int]:
        """
        Uses a depth-first search (DFS) to find_root a path from the start label to the end label, where all edges have a value
        for "key" that is greater than zero.
    ...
    def display_graphs(self,
                       ...
                       path_display: DirectedGraph=None) -> None:
        """
